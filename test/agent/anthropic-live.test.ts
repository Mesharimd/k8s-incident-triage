...
import { AnthropicProvider } from "../../src/agent/anthropic";
...
test.skipIf(Bun.env.ANTHROPIC_LIVE_TEST !== "1")(
  "Anthropic live API completes one strict synthetic tool-use round trip",
  async () => {
    const apiKey = Bun.env.ANTHROPIC_API_KEY?.trim();
    if (apiKey === undefined || apiKey.length === 0) {
      throw new Error("ANTHROPIC_API_KEY is required for the live provider test");
    }
    ...
    const toolTurn = await provider.complete(initialMessages, [inspectFixtureTool]);
    ...
    if (toolCall?.type !== "tool_call") {
      throw new Error("Anthropic live response must request inspect_fixture");
    }
